...
ADMIN_BOT_TOKEN = os.environ.get('ADMIN_BOT_TOKEN') # Admin Bot Token
ADMIN_CHAT_ID = os.environ.get('ADMIN_ID')         # သင့် Telegram ID
...
def check_user_joined(user_id):
    try: return bot.get_chat_member(CHANNEL_ID, user_id).status in ['creator', 'administrator', 'member']
    except: return False
...
def spin_game(message):
    ...
    # Matching Values: 1=🍒, 22=🍋, 43=7️⃣, 64=🍫
...
def handle_all_messages(message):
    if message.chat.type in ['group', 'supergroup']:
        if not check_user_joined(message.from_user.id):
            ...
            bot.send_message(message.chat.id, f"⚠️ Hello {get_user_display(message.from_user)}, join channel first!", reply_markup=markup, parse_mode="Markdown")
            return
